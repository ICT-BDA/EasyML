...
import bda.studio.client.ui.panel.LoginPanel;
import bda.studio.client.ui.panel.RegisterPanel;
import bda.studio.client.ui.panel.ResetpwdPanel;
import bda.studio.client.util.Constants;
import bda.studio.client.util.Util;
				...
				if (result != null) {
					if(result.getEmail() != "guest"){
						...
						if(result.getEmail().equals("admin") || result.getEmail().equals("guest")){
							...
							RootPanel.get("bda-account").getElement().setAttribute("href", "BDAStudio.html#account");
						}
					}else{
						RootPanel.get("bda-login").getElement().getStyle().setDisplay(Display.INLINE);
					}
			...
			accountSrv.autoLogin(account, new AsyncCallback<Account>() {
				...
				public void onSuccess(Account account) {
					if (account != null) {
						...
						if (refer == null)
							AppController.redirect("BDAStudio.html");
						else
