...
public class DatasetTreeLoader {
  ...
  public static void addDatasetLeaf(final DatasetTree tree,
	  ...
	  //logger.info("dataset tree loader category:"+category);
		...
		query.setType("data");
	 	categorySrv.getCategory(query, "", new AsyncCallback<List<Category>>(){
			...
			public void onSuccess(List<Category> result) {
				// TODO Auto-generated method stub
				//logger.info("dataset tree loader category result:"+result.size());
				if(result != null){
					TreeItem treeItem;
					if(result.get(0).getPath().equals(null)){
						//logger.info("category name:"+result.get(0).getName());
						treeItem = tree.findTreeItem(result.get(0).getName());
					}else
						//logger.info("category path:"+result.get(0).getName());
