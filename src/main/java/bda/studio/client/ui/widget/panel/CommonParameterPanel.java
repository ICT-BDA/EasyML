 ...
 */
package bda.studio.client.ui.widget.panel;

import java.util.List;

import bda.studio.client.ui.widget.command.Parameter;
import bda.studio.client.ui.widget.command.ValueCheck;
...
public class CommonParameterPanel extends ParameterPanel {
  ...
  public void cloneValue(CommonParameterPanel panel){
    List<Parameter> params = widget.getProgramConf().getParameters();
    List<Parameter> newparas = panel.getWidget().getProgramConf().getParameters();
    for (int i = 0; i < params.size(); ++i) {
      Parameter p = params.get(i);
      Parameter newp = newparas.get(i);
      if (p.getParamType().equals("Boolean")) {
        ListBox box = (ListBox) paramBoxs.get(i);
